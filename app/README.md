...
Onder `Vrijgave` beheert management het formele go-livedossier voor database/herstel, Entra, orderkoppeling, compatibiliteitsbewijs en werkvloeracceptatie. Iedere poort toont vier concrete bewijspunten; open acties worden automatisch gebundeld. Een goedkeuring vereist vier bevestigde controles, een bewijsreferentie en bewijsdatum; alleen vijf actuele goedkeuringen heffen de productieblokkade op. Het volledige dossier kan als versieerbare JSON of via de afdrukfunctie als PDF worden overgedragen. Migratie `0015_go_live_acceptance.sql` en `GET/POST /api/operations/go-live-acceptance` verzorgen de persoonlijke centrale audit en serverberekende status. CI registreert alleen een open controle en simuleert geen externe goedkeuring. Zie `../docs/GO_LIVE_ACCEPTANCE_DOSSIER.md`.

Onder `Werkvloerproef` plant en registreert management echte proeven met scanner, desktop/tablet, deelnemers, doorlooptijd, alle vier methoden en een foutscenario. Een geslaagd resultaat vereist minimaal vijf orders, zes afzonderlijke werkvloercontroles en een herleidbare bewijsreferentie. Het proefbewijs keurt de aparte go-livepoort nooit automatisch goed. Migratie `0016_workfloor_acceptance_trials.sql` en `GET/POST /api/operations/workfloor-trials` verzorgen de persoonlijke centrale audit. CI registreert alleen een open proef en maakt geen vals acceptatiebewijs. Zie `../docs/WORKFLOOR_ACCEPTANCE_TRIALS.md`.

Onder `Scenariotest` voert management 29 deterministische normale, grens- en foutscenario’s opnieuw uit. De matrix bestrijkt de vier conversiemethoden, bedraggrenzen, modelresolutie, hangmap 75, nul- en negatieve voorraad, ontvangst, E1/E2-/pasvormblokkades, werknemersrechten en formele vrijgave. Een JSON-export bewaart de actor, uitvoeringstijd en alle uitkomsten. Scenario’s die echte apparatuur of pasvorm vereisen blijven expliciet gemarkeerd als fysiek te bevestigen. Zie `../docs/OPERATIONAL_SCENARIOS.md`.
