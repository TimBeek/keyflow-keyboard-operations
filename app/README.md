...
Onder `Vrijgave` beheert management het formele go-livedossier voor database/herstel, Entra, orderkoppeling, compatibiliteitsbewijs en werkvloeracceptatie. Een goedkeuring vereist vier bevestigde controles, een bewijsreferentie en bewijsdatum; alleen vijf actuele goedkeuringen heffen de productieblokkade op. Migratie `0015_go_live_acceptance.sql` en `GET/POST /api/operations/go-live-acceptance` verzorgen de persoonlijke centrale audit. CI registreert alleen een open controle en simuleert geen externe goedkeuring. Zie `../docs/GO_LIVE_ACCEPTANCE_DOSSIER.md`.
