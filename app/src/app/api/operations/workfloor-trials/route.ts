import { workfloorTrialSummary } from "@/domain/workfloor-acceptance";
import { DatabaseConfigurationError } from "@/server/database";
import {
  listWorkfloorTrials,
  recordWorkfloorTrial,
  workfloorTrialErrorResponse,
} from "@/server/workfloor-acceptance-service";
import {
...
export async function GET(request: Request) {
  try {
    ...
    const records = await listWorkfloorTrials(actorId);
    return Response.json({
      records,
      summary: workfloorTrialSummary(records),
    });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const result = await recordWorkfloorTrial({
      ...body,
...
function routeErrorResponse(error: unknown) {
  ...
  const response = workfloorTrialErrorResponse(error);
  return Response.json(response.body, { status: response.status });
}
